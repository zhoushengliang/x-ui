...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/zhoushengliang/x-ui/main/x-ui.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update() {
    ...
    bash <(curl -Ls https://raw.githubusercontent.com/zhoushengliang/x-ui/main/install.sh)
    if [[ $? == 0 ]]; then
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://raw.githubusercontent.com/zhoushengliang/x-ui/main/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
